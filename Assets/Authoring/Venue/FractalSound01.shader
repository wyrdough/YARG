Shader "FractalSound01"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            int fractal(float2 p, float2 coord)
            {
                float2 so = (-1.0 + 2.0 * coord) * 0.4;
            ...
            float sampleMusic()
            {
                // return 0.5 * (_Yarg_SoundTex.Sample(sampler_Yarg_SoundTex, float2(0.15, 0.25)).x +
                //     _Yarg_SoundTex.Sample(sampler_Yarg_SoundTex, float2(0.30, 0.25)).x);
                return 0.5 * (_Yarg_SoundTex.Load(float3(0.15, 0.25, 0.0)).x + _Yarg_SoundTex.Load(float3(0.30, 0.25, 0.0)).x);
            }
            ...
            float4 mainImage(float2 fragCoord)
            {
                ...
                float2 position = 3 * (-0.5 + uv);
                position.x *= iResolution.x / iResolution.y;

                float2 iFC = float2(iResolution.x - fragCoord.x, iResolution.y - fragCoord.y);
                float2 pos2 = 2 * (-0.5 * iFC.xy / iResolution.xy);
                pos2.x = iResolution.x / iResolution.y;

                // float4 t3 = _Yarg_SoundTex.Sample(sampler_Yarg_SoundTex, float2(length(position) / 2.0, 0.1));
                float4 t3 = _Yarg_SoundTex.Load(float3(length(position) / 2.0, 0.1, 0.0));
                float pulse = 0.5 + sampleMusic() * 1.8;

                float v = 0.55 + sin(iTime / 3 + 0.5) / 2.0;
                float w = pulse * 0.9;
                float3 invFrac = color(fractal(pos2, float2(v, w)));

                float x = 0.6 + cos(iTime / 2 + 0.5);
                float y = pulse * 0.8;
                float3 fract4 = color(fractal(position / 1.6, float2(x, y)));

                float z = 0.5 + sin(iTime / 3) / 2;
                float3 c = color(fractal(position, float2(z, pulse)));

                t3 = abs(float4(0.5, 0.1, 0.5, 1) - t3) * 2;

                float4 fract01 = float4(c, 1.0);
                return fract01 / t3 + fract01 * t3 + float4(invFrac, 0.6) + float4(fract4, 0.3);
            }
