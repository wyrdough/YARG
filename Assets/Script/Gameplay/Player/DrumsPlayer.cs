...
namespace YARG.Gameplay.Player
{
    public class DrumsPlayer : TrackPlayer<DrumsEngine, DrumNote>
    {
        ...
        protected override void OnNoteHit(int index, DrumNote note)
        {
            ...
            {
                _fretArray.PlayDrumAnimation(GetFretForNote(note), true);
            }
            else
            {
                _kickFretFlash.PlayHitAnimation();
        ...
        protected override void InitializeTrackEffects()
        {
            base.InitializeTrackEffects();
            // Assign activation lanes to any drum fill effects generated by base

            // Assign the lane highlight for SP activation events
            int noteIndex = 0;

            foreach (var effect in _upcomingEffects)
            {
                if (effect.EffectType is not (TrackEffectType.DrumFill or
                    TrackEffectType.SoloAndDrumFill or
                    TrackEffectType.DrumFillAndUnison))
                {
                    continue;
                }

                // Find the last note in the drum fill
                for (; noteIndex < Notes.Count && Notes[noteIndex].Time <= effect.TimeEnd; noteIndex++)
                {

                }
                // Realistically, the activation note should never be the last note in the chart, but people
                // like to do stupid stuff.
                var activationNote = noteIndex == Notes.Count - 1 ? Notes[noteIndex] : Notes[noteIndex - 1];
                if (activationNote.IsStarPowerActivator)
                {
                    var fret = GetFretForNote(activationNote);
                    if (activationNote.Pad == (int) FourLaneDrumPad.Kick)
                    {
                        var candidateNote = activationNote.ParentOrSelf;
                        fret = GetFretForNote(candidateNote);
                        if (candidateNote.IsParent)
                        {
                            foreach (var child in candidateNote.ChildNotes)
                            {
                                var newFret = GetFretForNote(child);
                                if (newFret > fret)
                                {
                                    fret = newFret;
                                }
        ...
        private int GetFretForNote(DrumNote note)
        {
            int fret;
